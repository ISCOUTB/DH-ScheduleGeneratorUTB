...
- **Alerta por materia (⚠️):** en la lista del detalle, junto al nombre de la materia aparece un ícono de alerta rojo cuando alguna de sus opciones (NRC) no está disponible (`_optionUnavailable`). Dos causas: **sin cupos** (`available ≤ 0` en vivo, o snapshot en 0 en generación/término pasado) o **ya no aparece en la oferta** (término actual con datos cargados pero el NRC no está en `Curso`). Ojo: Banner normalmente **saca** de la oferta las secciones llenas en vez de mostrar "0 cupos", así que en la práctica el caso real casi siempre es el segundo (y "ya no aparece" suele significar que se llenó). El check de `available ≤ 0` se mantiene igual, como defensa. El ícono tiene un `Tooltip` (hover en escritorio, long-press en móvil) que explica el motivo concreto vía `_unavailableReason`. Si el estado no se pudo consultar (mapa vacío en el término actual) no se marca en rojo: no se sabe.
...
- **Navegación en el detalle (generación, escritorio) — panel de paginación:** toda la navegación vive en un panel al **pie de la columna derecha** del detalle (la de la lista de materias). `ScheduleOverviewWidget` recibe un `footer` opcional que se pinta ahí; lo arma `home_screen._buildOverviewPaginationPanel` y **solo** se pasa en generación y escritorio (en destacados y en móvil va `null`). El panel tiene:
  - **Centro:** flechas ◀ ▶ que pasan de horario en horario **dentro de la página** (`selectPrevInPage` / `selectNextInPage`, acotadas a `[_pageStartIndex, _pageEndIndex)`; se deshabilitan en los extremos vía `canSelectPrevInPage` / `canSelectNextInPage`), con el texto `Horario N de M` (índice global).
  - **Extremos:** "Página anterior" / "Página siguiente" (rótulo en dos líneas para no comerse el ancho). **Siempre activos** mientras exista página en ese sentido — no dependen de estar en el primer/último horario (`canGoToPrevPageFromOverview` / `canGoToNextPageFromOverview` solo miran `currentPage` vs `totalPages`). Saltan a la página destino y seleccionan su **primer** horario; el detalle sigue abierto.
  - **Debajo:** indicador `Página X de Y`, para ubicarse.
  - Las teclas ← → siguen moviendo horario a horario: un `Focus` ancestro las captura aunque el foco esté en un botón interno (el evento sube antes del traversal direccional). El detalle se reconstruye con `Key: ValueKey(selectedScheduleIndex)` para recalcular su estado (numeración de materias, etc.).
- **Cerrar el detalle con `Esc` (escritorio):** funciona en los dos detalles. En generación lo maneja el mismo `Focus` del overlay (`closeScheduleOverview`); en destacados, el `Focus` que envuelve el body (`_handleDesktopKey`), que solo lo consume si el detalle está abierto y si no deja pasar la tecla. Complementa al clic sobre el fondo oscuro, que ya cerraba en escritorio.
- **Navegación en destacados (escritorio):** con las teclas ↑ ↓ se pasa entre los horarios destacados, **tanto en la lista como dentro del detalle** (mueve `_selectedIndex` en `FavoritesScreen`, clamp a la lista). En ambos casos se desplaza el sidebar para mantener centrada (visible) la tarjeta seleccionada (`Scrollable.ensureVisible` sobre una `GlobalKey` por tarjeta); dentro del detalle además se recargan los cupos del nuevo horario y el modal se reconstruye con `Key: ValueKey(_selectedIndex)`. Un `Focus` ancestro captura las teclas aunque el foco esté en una tarjeta. No hay flechas en pantalla en destacados.
...
- **Jerarquía del panel de filtros (`filter_widget.dart`):** las secciones ("Filtros por Materia", "Filtro por NRC") son `ExpansionTile` con materias anidadas dentro, también `ExpansionTile`. Sin ayuda, esa anidación se lee **al revés**: el `leading` del padre le reserva ~56px y empuja su título, mientras que los hijos caen en el `tilePadding` por defecto (16), o sea **a la izquierda del título del padre**. Tres piezas lo corrigen, y conviene no quitarlas por separado:
  - `_sectionTitle` — el título de sección va en negrita; antes padre e hijos pesaban igual.
  - `_nestedGroup` — línea guía vertical a la izquierda del grupo. Es lo que realmente comunica la anidación: por más que se indente, el texto de un hijo nunca va a arrancar más a la derecha que el del padre sin comerse media pantalla, así que la agrupación se resuelve con la línea, no alineando.
  - `_sectionChildrenPadding` / `_nestedTilePadding` — meten a los hijos por **ambos** lados. La derecha lleva más que el default a propósito: deja el chevron del hijo por dentro del chevron del padre. Con la derecha al ras, el bloque hijo se leía desbordado en vez de contenido. Costo: la fila del hijo queda más angosta, o sea menos espacio para nombres de materia largos.
